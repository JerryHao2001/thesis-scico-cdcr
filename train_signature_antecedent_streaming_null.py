...
def marginal_nll_for_mention_null(
    all_logits: torch.Tensor,         # [1+m] where 0 is NULL action
    cand_ids: List[int],              # len m (does NOT include NULL)
    gold_ants: List[int],             # subset of cand_ids
) -> torch.Tensor:
    """Marginal NLL when epsilon is represented as a NULL antecedent.

    Actions are: 0 = NULL ("no antecedent"), 1..m = candidates aligned to cand_ids.
    If there is no gold antecedent, correct action is NULL.
    """
    log_probs = torch.log_softmax(all_logits, dim=0)
    ...
    for g in gold_ants:
        if g in idx_map:
            pos.append(1 + idx_map[g])
    if len(pos) == 0:
        return -log_probs[0]
    return -(torch.logsumexp(log_probs[torch.tensor(pos, device=log_probs.device)], dim=0))


def _reduce_cluster_logits(
    logits: torch.Tensor,
    reduce: str,
) -> torch.Tensor:
    """Reduce a 1D tensor of member logits into a single cluster logit."""
    if logits.numel() == 1:
        return logits.view(())
    if reduce == "max":
...
def decode_topic(
    ...
    amp: bool,
    seed: int,
    eps_logit: Optional[torch.Tensor] = None,
    null_sig: Optional[str] = None,
    decode_mode: str = "greedy",
    link_margin: float = 0.0,
    cluster_reduce: str = "max",
) -> List[int]:
    """Decode a topic into predicted cluster labels.

    decode_mode:
      - "greedy": standard mention-ranking decode (Lee et al.): pick best antecedent
        among candidates vs ε; cluster is transitive closure of chosen links.
      - "cluster": cluster-aware decode. At each step, aggregate evidence from
        *predicted* clusters among candidates using `cluster_reduce` and pick the
        best cluster vs ε.

    link_margin:
      Extra conservatism for linking. A link is made only if the best action's
      logit exceeds ε by at least `link_margin`.
      (When decode_mode="cluster", the comparison uses the aggregated cluster logit.)
    """
    if decode_mode not in {"greedy", "cluster"}:
    ...
    for i in range(len(signatures)):
        ...
        # NULL-epsilon mode: compare best candidate(s) vs NULL logit.
        if null_sig is not None:
            if len(cand_sigs) == 0:
                continue  # only NULL available -> no link
            sig_js = [null_sig] + cand_sigs
            with torch.no_grad():
                all_logits = score_pairs(
                    model,
                    tokenizer,
                    signatures[i],
                    sig_js,
            ...
            null_logit = all_logits[0]
            cand_logits = all_logits[1:]

            if decode_mode == "greedy":
                best_cand_val, best_cand_pos = torch.max(cand_logits, dim=0)
                if float(best_cand_val.item()) <= float(null_logit.item()) + float(link_margin):
                    continue
                j = cand_ids[int(best_cand_pos.item())]
                uf.union(i, j)
                continue

            # cluster-aware NULL decode
            by_root: Dict[int, List[int]] = {}
            for pos, j in enumerate(cand_ids):
                r = uf.find(j)
                by_root.setdefault(r, []).append(pos)

            roots: List[int] = []
            root_logits: List[torch.Tensor] = []
            root_best_member: Dict[int, int] = {}

            for r, positions in by_root.items():
                roots.append(r)
                member_logits = cand_logits[torch.tensor(positions, device=cand_logits.device)]
                root_logits.append(_reduce_cluster_logits(member_logits, reduce=cluster_reduce))
                best_pos_in_cluster = positions[int(torch.argmax(member_logits).item())]
                root_best_member[r] = cand_ids[best_pos_in_cluster]

            cluster_logits = torch.stack(root_logits, dim=0)
            best_cluster_val, best_cluster_pos = torch.max(cluster_logits, dim=0)
            if float(best_cluster_val.item()) <= float(null_logit.item()) + float(link_margin):
                continue
            best_root = roots[int(best_cluster_pos.item())]
            uf.union(i, root_best_member[best_root])
            continue

        # Scalar-epsilon mode (legacy): compare best candidate(s) vs eps_logit.
        if eps_logit is None:
            raise ValueError("decode_topic: eps_logit is required when null_sig is None")
        if not cand_ids:
            continue
        with torch.no_grad():
        ...
        if decode_mode == "greedy":
            best_cand_val, best_cand_pos = torch.max(cand_logits, dim=0)
            # Link only if best candidate beats ε by margin.
            if float(best_cand_val.item()) <= float(eps_logit.item()) + float(link_margin):
                continue
            j = cand_ids[int(best_cand_pos.item())]
            uf.union(i, j)
            continue

        # cluster-aware decoding
        # Group candidate logits by current predicted cluster.
        by_root: Dict[int, List[int]] = {}
        for pos, j in enumerate(cand_ids):
            r = uf.find(j)
            by_root.setdefault(r, []).append(pos)
        ...
        for r, positions in by_root.items():
            ...
            member_logits = cand_logits[torch.tensor(positions, device=cand_logits.device)]
            root_logits.append(_reduce_cluster_logits(member_logits, reduce=cluster_reduce))
            # We'll union i with the highest-logit member in the chosen cluster.
            best_pos_in_cluster = positions[int(torch.argmax(member_logits).item())]
            root_best_member[r] = cand_ids[best_pos_in_cluster]

        cluster_logits = torch.stack(root_logits, dim=0)  # [num_clusters]
        best_cluster_val, best_cluster_pos = torch.max(cluster_logits, dim=0)

        if float(best_cluster_val.item()) <= float(eps_logit.item()) + float(link_margin):
            continue
        best_root = roots[int(best_cluster_pos.item())]
        uf.union(i, root_best_member[best_root])
...
def decode_topic_greedy(
    ...
    """Backward-compatible wrapper for greedy decoding."""
    return decode_topic(
        model=model,
        tokenizer=tokenizer,
...
def main():
    ...
    ap.add_argument(
        "--tokenizer_name",
        default=None,
        help="Tokenizer name/id. Default: use --bert_model. "
             "Set this if your backbone uses a different tokenizer (rare).",
    )
    ap.add_argument("--adapter_name", default=None, help="Optional HF adapter id, e.g. allenai/specter2")
    ap.add_argument("--max_length", type=int, default=386)
    ...
    ap.add_argument(
        ...
        choices=["scalar", "null"],
        default="null",
        help="How to represent the dummy antecedent (epsilon). 'scalar' uses a global eps_logit; 'null' uses a NULL antecedent signature scored by the cross-encoder.",
    )
    ap.add_argument(
        "--null_sig",
        type=str,
        default="<NULL_ANT> no antecedent </NULL_ANT>",
        help="NULL antecedent signature used when eps_mode='null'.",
    )
    ...
    ap.add_argument(
        "--decode_mode",
        default="greedy",
        choices=["greedy", "cluster"],
        help="Decoding algorithm used for validation CoNLL (and recommended for test eval).",
    )
    ap.add_argument(
        "--decode_link_margin",
        type=float,
        default=0.0,
        help="Only create a link if best action logit > eps_logit + margin. "
             "Useful to tune merge aggressiveness on dev.",
    )
    ap.add_argument(
        "--decode_cluster_reduce",
        default="max",
        choices=["max", "mean", "logsumexp", "logmeanexp"],
        help="When --decode_mode=cluster, how to aggregate mention-to-mention logits into a cluster logit.",
    )
    ...
    # This trainer performs ONE optimizer update per TOPIC.
    # Keep topic_batch_size=1 to keep scheduler/update semantics correct and standard.
    if args.topic_batch_size != 1:
        raise ValueError(
            f"topic_batch_size must be 1 for this streaming trainer (got {args.topic_batch_size}). "
            "If you want true batching across topics, refactor the optimizer/scheduler stepping."
        )

    # Mention-weighted training: larger topics should contribute proportionally more signal.
    # We sum per-mention losses within a topic, and divide by a DATASET-LEVEL constant
    # (avg mentions/topic) to keep gradient magnitudes roughly stable across runs.
    total_train_mentions = 0
    for i in range(n_train_topics):
        ex = train_ds[i]
        total_train_mentions += len(ex["signatures"])
    avg_mentions_per_topic = max(1.0, total_train_mentions / float(n_train_topics))
    print(
        f"[data] train mentions total={total_train_mentions:,}  avg/topic={avg_mentions_per_topic:.2f}"
    )
    ...
    # Tokenizer (must be consistent between training and eval).
    tok_name = args.tokenizer_name or args.bert_model
    tok = build_signature_tokenizer(tok_name, use_fast=True)

    # If using NULL-epsilon mode, ensure NULL tags are treated as special tokens.
    if args.eps_mode == "null":
        for t in ["<NULL_ANT>", "</NULL_ANT>"]:
            if t not in SIGNATURE_SPECIAL_TOKENS:
                SIGNATURE_SPECIAL_TOKENS.append(t)
    ...
    # Ensure the model's embedding matrix includes signature special tokens.
    # IMPORTANT: tokenizer must already have had tokens added in the canonical order.
    ...
    # Always train the full model (backbone + adapter, if any + head).
    # This avoids underfitting and is consistent with your observations.
    for p in model.parameters():
        p.requires_grad = True

    model.to(device)

    # Dummy antecedent representation
    # - eps_mode='scalar': a global eps_logit (optionally trainable)
    # - eps_mode='null': no scalar; epsilon is a NULL antecedent signature scored by the same cross-encoder
    if args.eps_mode == "scalar":
        if args.train_eps:
            eps_logit: Optional[torch.Tensor] = torch.nn.Parameter(torch.tensor(float(args.eps_init), device=device))
    ...
    # One optimizer update per topic -> schedule should be based on number of updates.
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # Track loss as a micro-average over mentions (for logging)
        running_loss = 0.0
        running_mentions = 0
        n_topics = 0
        ...
        for batch_topics in train_dl:
            ...
            for topic in batch_topics:
                ...
                # loss_sum_raw: unnormalized sum of per-mention NLLs in this topic
                # mention_count: how many mentions contributed a loss term
                loss_sum_raw = 0.0
                mention_count = 0
                ...
                for i in range(n):
                    ...
                    if args.eps_mode == "scalar" and len(cand_ids) == 0:
                        # Only eps action exists; loss would be 0 for new entities.
                        continue
                    ...
                    if args.eps_mode == "null":
                        # NULL-epsilon: prepend NULL candidate and compute one set of logits
                        sig_js = [args.null_sig] + cand_sigs
                        all_logits = score_pairs(
                            model=model,
                            tokenizer=tok,
                            sig_i=signatures[i],
                        ...
                        loss_i_raw = marginal_nll_for_mention_null(
                            all_logits=all_logits,
                            cand_ids=cand_ids,
                            gold_ants=gold_ants,
                        )
                    else:
                        # scalar-epsilon legacy
                        cand_logits = score_pairs(
                            model=model,
                            tokenizer=tok,
                            sig_i=signatures[i],
                    ...
                    # Mention-weighted objective: sum per-mention losses within a topic.
                    # Normalize by dataset-level avg mentions/topic to keep step sizes stable.
                    loss_i = loss_i_raw / avg_mentions_per_topic

                    scaler.scale(loss_i).backward()
                    loss_sum_raw += float(loss_i_raw.detach().item())
                    mention_count += 1
                ...
                running_loss += loss_sum_raw
                running_mentions += mention_count

                # tqdm update per topic
                try:
                    pbar.update(1)
                    lr = float(optimizer.param_groups[0]["lr"])
                    avg_loss_micro = running_loss / max(1, running_mentions)
                    topic_loss_micro = loss_sum_raw / max(1, mention_count)
                    pbar.set_postfix({
                        "avg_loss": f"{avg_loss_micro:.4f}",
                        "topic_loss": f"{topic_loss_micro:.4f}",
                        "lr": f"{lr:.2e}",
                    })
                except Exception:
                    pass
        ...
        avg_loss = running_loss / max(1, running_mentions)
        print(
            f"[epoch {epoch}] train_loss={avg_loss:.6f} topics={n_topics} mentions={running_mentions}"
        )
            ...
            with torch.no_grad():
                for batch_topics in val_dl:
                    ...
                    pred_labels = decode_topic(
                        ...
                        eps_logit=(eps_logit if torch.is_tensor(eps_logit) else (torch.tensor(float(eps_logit), device=device) if eps_logit is not None else None)),
                        null_sig=(args.null_sig if args.eps_mode == "null" else None),
                        device=device,
                        max_length=args.max_length,
                        pair_batch_size=args.pair_batch_size,
                        amp=use_amp,
                        ...
                        link_margin=args.decode_link_margin,
                        cluster_reduce=args.decode_cluster_reduce,
                    )
        ...
        if args.save_every_epoch or (conll is not None and conll > best_conll):
            ckpt_path = os.path.join(args.output_dir, f"epoch{epoch}_{conll:.3f}.pt")
            ckpt_payload = {
            ...
            if args.eps_mode == "scalar":
                ckpt_payload["eps_logit"] = float(eps_logit.detach().item()) if torch.is_tensor(eps_logit) else float(eps_logit)
            else:
                ckpt_payload["null_sig"] = args.null_sig
